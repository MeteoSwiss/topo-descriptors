...
def compute_tpi(dem_da, scales, smth_factors=None, ind_nans=[], crop=None):
    ...
    if not hasattr(scales, '__iter__'): scales = [scales]
    if not hasattr(smth_factors, '__iter__'): smth_factors = [smth_factors] * len(scales)
...
def tpi(dem, size, sigma=None):
    ...
    conv = signal.convolve(dem, kernel, mode='same') # ndimage.convolve(dem, kernel, mode='reflect')
...
def compute_valley_ridge(dem_da, 
                         scales,
                         mode,
...
def _valley_kernels(size, flat_list):
    ...
    for ind,flat in enumerate(flat_list):
        ...
        kernels[ind,middle-halfwidth:middle+halfwidth+1,:] = kernels[ind,middle-halfwidth,0]
        kernels = (kernels - np.mean(kernels,axis=(1,2),keepdims=True))/np.std(kernels,axis=(1,2),keepdims=True)
...
def _rotate_kernels(kernel, angle): 
    ...
    kernels_rot = ndimage.rotate(kernel, angle , axes=(1,2), reshape=True, order=2, mode='constant', cval=-9999)
    kernels_rot = ma.masked_array(kernels_rot, mask=kernels_rot == -9999)
    kernels_rot = (kernels_rot - np.mean(kernels_rot,axis=(1,2),keepdims=True))/np.std(kernels_rot,axis=(1,2),keepdims=True)
...
def compute_gradient(dem_da, scales, sig_ratios=1, ind_nans=[], crop=None):
    ...
    if not hasattr(scales, '__iter__'): scales = [scales]
    if not hasattr(sig_ratios, '__iter__'): sig_ratios = [sig_ratios] * len(scales)

    scales_pxl, res_meters = hlp.scale_to_pixel(scales, dem_da)
    sigmas = scales_pxl / 4 # scale = 4std
    
    for idx, sigma in enumerate(sigmas):
        logger.info(f'Computing scale {scales[idx]} meters with sigma ratio '
                    f'{sig_ratios[idx]} ...')
        names = _gradient_names(scales[idx], sig_ratios[idx])
        arrays = gradient(dem = dem_da.values, 
                          sigma = sigma,
...
def gradient(dem, sigma, res_meters, sig_ratio=1):
    ...
    aspect = (180 + np.degrees(np.arctan2(dx, dy))) % 360  # north faces = 0°, east faces = 90° 
